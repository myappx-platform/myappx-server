	...
	protected String doIt() throws Exception
	{
		...
		try {
			...
			sql="UPDATE AD_FIELD_TRL trl"
				...
				+"			  AND c.AD_Column_ID=f.AD_Column_ID AND f.AD_Field_ID=trl.AD_Field_ID AND e.AD_Client_ID=trl.AD_Client_ID),"
				...
				+"		  AND e.AD_Client_ID=trl.AD_Client_ID"
			...
			sql="UPDATE AD_FIELD_TRL trl"
				...
				+" 			AND f.AD_Field_ID=trl.AD_Field_ID AND p.AD_LANGUAGE=trl.AD_LANGUAGE AND p.AD_Client_ID=trl.AD_Client_ID),"
				...
				+" 		AND p.AD_Client_ID=trl.AD_Client_ID"
				+" 		AND f.IsCentrallyMaintained='Y' AND f.IsActive='Y'"
			...
			sql="UPDATE AD_PROCESS_PARA_TRL trl"
				...
				+" 			  AND e.ColumnName=f.ColumnName AND f.AD_Process_Para_ID=trl.AD_Process_Para_ID AND et.AD_Client_ID=trl.AD_Client_ID),"
				...
				+" 			  AND et.AD_Client_ID=trl.AD_Client_ID"
				+" 			  AND f.IsCentrallyMaintained='Y' AND f.IsActive='Y'"
			...
			sql="UPDATE AD_INFOCOLUMN_TRL trl"
				...
				+" 			WHERE et.AD_LANGUAGE=trl.AD_LANGUAGE AND et.AD_Element_ID=e.AD_Element_ID"
				+" 			  AND e.ColumnName=f.ColumnName AND f.AD_InfoColumn_ID=trl.AD_InfoColumn_ID AND et.AD_Client_ID=trl.AD_Client_ID),"
			...
			sql="UPDATE AD_PRINTFORMATITEM_TRL trl"
				...
				+"			WHERE e.AD_LANGUAGE=trl.AD_LANGUAGE AND e.AD_Element_ID=c.AD_Element_ID "
				+"			  AND c.AD_Column_ID=p.AD_Column_ID AND p.AD_PrintFormatItem_ID=trl.AD_PrintFormatItem_ID AND e.AD_Client_ID=trl.AD_Client_ID),"
				+"	Updated = getDate()"
			...
			sql="UPDATE AD_PRINTFORMATITEM_TRL trl"
				...
				+" AND c.AD_Column_ID=pfi.AD_Column_ID"
				+" AND pfi.AD_PrintFormatItem_ID=trl.AD_PrintFormatItem_ID AND e.AD_Client_ID=trl.AD_Client_ID)"
			...
			sql="UPDATE AD_COLUMN_TRL ct"
				...
				+" WHERE ct.AD_Column_ID=c.AD_Column_ID AND ct.AD_LANGUAGE=e.AD_LANGUAGE AND e.AD_Client_ID=ct.AD_Client_ID)"
				+" WHERE EXISTS "
			...
			sql="UPDATE AD_TABLE_TRL tt" 
				...
				+" WHERE tt.AD_Table_ID=t.AD_Table_ID AND tt.AD_LANGUAGE=e.AD_LANGUAGE AND e.AD_Client_ID=tt.AD_Client_ID)"
				+" WHERE EXISTS (SELECT 1 "
				+" FROM AD_TABLE t INNER JOIN AD_ELEMENT ex ON (t.TableName||'_ID'=ex.ColumnName)"
				+" INNER JOIN AD_ELEMENT_TRL e ON (ex.AD_Element_ID=e.AD_Element_ID)"
				+" WHERE tt.AD_Table_ID=t.AD_Table_ID AND tt.AD_LANGUAGE=e.AD_LANGUAGE AND e.AD_Client_ID=tt.AD_Client_ID"
