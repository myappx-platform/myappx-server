...
billboard.TreemapRenderer = function() {};

billboard.TreemapRenderer.prototype.render = function(wgt) {
	var columns = wgt.getSeriesData()[0];
	var color = {};
	var treemap = { tile: "binary" };
	var background = {};
	...
	if (rendererOptions) {
		...
		if (rendererOptions["treemap"]) {
			treemap = jq.extend(true, treemap, rendererOptions["treemap"]);
		}
	}
	var seriesColors = wgt.getSeriesColors() ? jq.evalJSON(wgt.getSeriesColors()) : null;
	if (seriesColors && seriesColors.length && !color["pattern"]) {
		color["pattern"] = seriesColors;
	}
...
zul.billboard.Billboard._renderers["treemap"] = new billboard.TreemapRenderer();
